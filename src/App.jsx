...
export default function App(){
  ...
  const fetchSnapshot = async () => {
    try {
      ...
      const mapped = rows.map(x => ({
        symbol:x.s || x.n, name:x.n || x.s, price:Number(x.p || 0), change:Number(x.m || 0), volume:Number(x.v || 0),
        momentum:Number(x.b || 0), score:Number(x.score || 0), setup:x.setup || x.status || 'WATCH', status:x.status || 'WATCH',
        resistance:Number(x.res || 0), support:Number(x.sup || 0), target1:Number(x.target1 || 0), target2:Number(x.target2 || 0),
        ret20:Number(x.ret20 || 0), risk_pct:Number(x.risk_pct || 0), rr1:Number(x.rr1 || 0), rr2:Number(x.rr2 || 0),
        retest_hold:Boolean(x.retest_hold), no_chase:Boolean(x.no_chase)
      }));
      setStocks(mapped);
      setSelectedStock(prev => prev || mapped[0] || null);
      setSnapshotUpdated(data.updated_utc || '');
      return true;
    } catch (err) { setApiError(err.message || 'Swing snapshot unavailable'); return false; }
  };
  ...
  const fetchLiveIndices = async () => {
    try {
      ...
      setIndices(data.rows || []); setLastUpdated(new Date().toLocaleTimeString('en-IN')); return true;
    } catch (err) { setApiError(err.message || 'Live data unavailable'); return false; }
  };
  ...
  const fetchStockAnalysis = async symbol => {
    if (!symbol) return;
    try {
      setStockAnalysisLoading(true); setStockAnalysisError('');
      const res = await fetch(`/api/stock-analysis?symbol=${encodeURIComponent(symbol)}&t=${Date.now()}`,{cache:'no-store'});
      const data = await res.json();
      if (!res.ok || !data.ok) throw new Error(data.error || 'Stock confirmation API failed');
      setStockAnalysis(data);
    } catch (err) { setStockAnalysis(null); setStockAnalysisError(err.message || 'Live stock confirmation unavailable'); }
    finally { setStockAnalysisLoading(false); }
  };

  useEffect(() => { fetchSnapshot(); const timer=setInterval(fetchSnapshot,30000); return()=>clearInterval(timer); },[]);
  useEffect(() => { if(!apiConnected)return; fetchLiveIndices(); const timer=setInterval(fetchLiveIndices,5000); return()=>clearInterval(timer); },[apiConnected]);
  useEffect(() => { if(mode==='stocks' && selectedStock?.symbol) fetchStockAnalysis(selectedStock.symbol); },[mode,selectedStock?.symbol]);
  useEffect(() => { if(mode!=='stocks'||!selectedStock?.symbol)return; const timer=setInterval(()=>fetchStockAnalysis(selectedStock.symbol),30000); return()=>clearInterval(timer); },[mode,selectedStock?.symbol]);
  ...
  const indexDirection = useMemo(() => { if(!priceAction)return 'WAIT'; const score=Number(priceAction.score||0); return score>=60?'LONG':score<=-60?'SHORT':'WAIT'; },[priceAction]);
  ...
  const setup = useMemo(() => {
    ...
    if(stockAnalysis?.grade==='A+' && stockDirection!=='WAIT') return {direction:stockDirection,entry:Number(stockAnalysis.entry||0),sl:Number(stockAnalysis.stop_loss||0),target1:Number(stockAnalysis.target1||0),target2:Number(stockAnalysis.target2||0),rr:stockAnalysis.risk_reward||'1 : 2'};
    return {direction:'WAIT',entry:0,sl:0,target1:0,target2:0,rr:'—'};
  },[mode,selectedIndexLive,indexDirection,stockAnalysis,stockDirection]);
  const quantity = useMemo(() => { if(setup.direction==='WAIT'||!setup.entry||!setup.sl)return 0; const riskAmount=capital*riskPercent/100; const unitRisk=Math.abs(setup.entry-setup.sl); return unitRisk>0?Math.floor(riskAmount/unitRisk):0; },[capital,riskPercent,setup]);
  const filteredStocks = stocks.filter(s => { const q=String(s.symbol).toLowerCase().includes(searchQuery.toLowerCase()); const v=Number(s.volume||0); return q&&(filterVolume==='all'||(filterVolume==='high'&&v>=3)||(filterVolume==='medium'&&v>=1.5&&v<3)); });

  const selectStock = stock => { setMode('stocks'); setSelectedStock(stock); setAiAnalysis(''); setStockAnalysis(null); setStockAnalysisError(''); fetchStockAnalysis(stock.symbol); };
  ...
  const handleAIAnalysis = () => {
    ...
    setTimeout(() => {
      if(mode==='indices'){
        ...
        const s=selectedStock; const a=stockAnalysis;
        setAiAnalysis(`🤖 ${s?.symbol||'STOCK'} MASTER REPORT\n\nLTP: ₹${money(s?.price)}\nA+ Grade: ${a?.grade||'—'}\nMTF Score: ${a?.score??'—'}\nBias: ${a?.bias||'WAIT'}\nStructure: ${a?.structure?.m5||'WAIT'}\nBOS: ${a?.bos?.m5||'WAIT'}\nRetest: ${a?.retest||'WAIT'}\nVolume Confirmed: ${a?.volume_confirmed?'YES':'NO'}\n\nFINAL VERDICT: ${a?.decision||'WAIT'}\n\n${a?.grade==='A+'?'✅ A+ confirmation. Verify broker price before execution.':'⚠️ Confirmation incomplete. NO TRADE.'}`);
      }
      setLoading(false);
    },300);
  };
...
function StockConfirmationPanel({data,loading,error,onRefresh}){const grade=data?.grade||'—';const gradeCls=grade==='A+'?'text-emerald-400':grade==='B'?'text-amber-400':'text-rose-400';return <div className="bg-slate-900 border-2 border-emerald-500/30 rounded-2xl p-5"><div className="flex justify-between items-center"><div><div className="text-xs text-emerald-400 font-bold">LIVE CONFIRMATION ENGINE • V1</div><h3 className="text-lg font-bold mt-1">A+ TRADE GATE</h3></div><button onClick={onRefresh} disabled={loading} className="p-2 rounded-lg bg-slate-800"><RefreshCw className={`w-4 h-4 ${loading?'animate-spin':''}`}/></button></div>{error&&<div className="mt-3 text-xs text-rose-300 bg-rose-950/30 border border-rose-500/30 rounded-xl p-3">⚠️ {error}</div>}{loading&&!data?<div className="mt-5 text-sm text-slate-400">Daily + 1H + 15M + 5M confirmation check ho rahi hai...</div>:data?<div className="mt-4 space-y-3"><div className={`text-center text-3xl font-extrabold ${gradeCls}`}>{grade}</div><div className="grid grid-cols-2 gap-2"><Metric label="Decision" value={data.decision||'WAIT'} accent={data.decision==='LONG'?'green':data.decision==='SHORT'?'red':'amber'}/><Metric label="MTF Score" value={`${data.score||0}/100`}/><Metric label="Structure" value={data.structure?.m5||'WAIT'}/><Metric label="BOS" value={data.bos?.m5||'WAIT'}/><Metric label="Retest" value={data.retest||'WAIT'}/><Metric label="Volume" value={data.volume_confirmed?'CONFIRMED':'NOT CONFIRMED'} accent={data.volume_confirmed?'green':'amber'}/></div><div className="bg-slate-950 rounded-xl p-3 text-xs text-slate-400">Rule: <b className="text-emerald-400">A+</b> = confirmation + volume + score ≥80. <b className="text-amber-400">B</b> = WATCH. <b className="text-rose-400">C</b> = AVOID.</div></div>:<div className="mt-5 text-sm text-slate-400">Stock select करें.</div>}</div>}
