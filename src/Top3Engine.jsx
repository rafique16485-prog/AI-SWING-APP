...
function grade(s){
  if(!s)return 'C';
  const score=Number(s.score||0),status=String(s.status||s.setup||'').toUpperCase();
  const aPlus=score>=70&&Boolean(s.retest_hold)&&!Boolean(s.no_chase)&&!['AVOID','WATCH','WAIT'].includes(status);
  if(aPlus)return 'A+';
  if(score>=60)return 'B';
  return 'C';
}
function confirmed(s){return grade(s)==='A+';}
function decision(s){
  const g=grade(s);
  if(g==='A+'){
    const d=direction(s);
...
export default function Top3Engine(){
 ...
 const indices=async()=>{try{const r=await fetch('/api/live-market?symbols=NIFTY50,SENSEX&t='+Date.now(),{cache:'no-store'}),d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||'Live API failed');setIndices(d.rows||[]);}catch(e){setError(e.message)}};
 ...
 useEffect(()=>{snapshot();const t=setInterval(snapshot,30000);return()=>clearInterval(t)},[]);
 ...
 const selectedPlan=useMemo(()=>selected&&confirmed(selected)?plan(selected,direction(selected)):null,[selected]);
 ...
 <div className="flex justify-between items-center"><div><h2 className="text-xl font-black flex gap-2 items-center"><Zap className="text-emerald-400"/>A+ CONFIRMATION ENGINE</h2><p className="text-xs text-slate-500">A+ = score ≥70 + retest hold + no-chase cleared + status confirmed. B = watch. C = avoid.</p></div><div className="text-[10px] text-slate-500">Snapshot: {updated?new Date(updated).toLocaleString('en-IN'):'loading'}</div></div>
 ...
 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">{top3.map((s,i)=>{const d=direction(s),g=grade(s),dec=decision(s),p=confirmed(s)?plan(s,d):null;return <button key={s.symbol} onClick={()=>setSelected(s)} className={'text-left bg-slate-900 border rounded-2xl p-5 '+(g==='A+'?'border-emerald-500/50':'border-slate-800')}><div className="flex justify-between"><span className="text-2xl">#{i+1}</span><span className={'text-xs font-bold px-2 py-1 rounded '+(g==='A+'?'bg-emerald-500/20 text-emerald-400':g==='B'?'bg-amber-500/10 text-amber-400':'bg-rose-500/10 text-rose-400')}>{g} • {dec}</span></div><h3 className="text-lg font-black mt-3">{s.symbol}</h3><div className="text-2xl font-mono font-bold mt-2">₹{money(s.price)}</div><div className="grid grid-cols-2 gap-2 mt-4 text-xs"><Metric a="Score" b={s.score}/><Metric a="Move" b={(s.change>=0?'+':'')+s.change.toFixed(2)+'%'}/><Metric a="Volume" b={s.volume.toFixed(2)+'x'}/><Metric a="Direction" b={d}/></div>{p?<div className="mt-4 space-y-2 text-xs"><Level a="Entry" b={p.entry}/><Level a="SL" b={p.sl}/><Level a="T1" b={p.target1}/><Level a="T2" b={p.target2}/><Level a="R:R" b={p.rr}/></div>:<div className={'mt-4 rounded-xl p-3 text-xs '+(g==='B'?'bg-amber-950/30 text-amber-300':'bg-slate-950 text-slate-400')}>{g==='B'?'🟡 WATCH — confirmation pending. No trade.':'🔴 AVOID — setup quality too weak.'}</div>}</button>})}</div>
 <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden"><div className="p-4 border-b border-slate-800 flex justify-between"><b>LIVE CANDIDATES • {stocks.length}</b><span className="text-xs text-slate-500">Auto refresh 30s</span></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-950 text-slate-500"><tr><th className="p-3 text-left">Stock</th><th className="p-3">LTP</th><th className="p-3">Move</th><th className="p-3">Vol</th><th className="p-3">Score</th><th className="p-3">Retest</th><th className="p-3">Grade</th><th className="p-3">Decision</th></tr></thead><tbody className="divide-y divide-slate-800">{stocks.slice(0,20).map(s=><tr key={s.symbol} onClick={()=>setSelected(s)} className="hover:bg-slate-800 cursor-pointer"><td className="p-3 font-bold">{s.symbol}</td><td className="p-3 text-center">₹{money(s.price)}</td><td className={'p-3 text-center '+(s.change>=0?'text-emerald-400':'text-rose-400')}>{s.change>=0?'+':''}{s.change.toFixed(2)}%</td><td className="p-3 text-center">{s.volume.toFixed(2)}x</td><td className="p-3 text-center font-bold">{s.score}</td><td className="p-3 text-center">{s.retest_hold?'HOLD':'—'}</td><td className="p-3 text-center font-bold">{grade(s)}</td><td className="p-3 text-center font-bold">{decision(s)}</td></tr>)}</tbody></table></div></div>
 {selected&&<div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="font-black text-lg">{selected.symbol} • Setup Decoder</h3><div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-4"><Metric a="Grade" b={grade(selected)}/><Metric a="Score" b={selected.score}/><Metric a="Setup" b={selected.setup}/><Metric a="Support" b={'₹'+money(selected.support)}/><Metric a="Resistance" b={'₹'+money(selected.resistance)}/></div><p className="text-xs text-slate-500 mt-4">A+ trade only after confirmation. B means WATCH; C means AVOID. No automatic order placement.</p></div>}
